...
void partial_corr(int cols, int* ifault, int in, int dimc, double* D, double* r, double* sserr, double* work, double* rhs, double* cormat, double* ycorr) {
  int base_pos, pos, pos1, pos2;
  double rms[cols-in-1];
  ...
  if(D[in+1] > 0.0)
    rms[0] = 1.0/sqrt(D[in+1]);
  for(int i=in+1; i<cols; i++) {
    pos = base_pos + i;
    sumxx = D[i];
    for(int j=in; j<cols-1; j++) {
      sumxx = sumxx+D[j]*pow(r[pos],2);
      pos = pos+cols-j-2;
    }
    if(sumxx > 0.0) {
      rms[i] = 1.0/sqrt(sumxx);
    } else {
      rms[i] = 0.0;
      ifault[0] = -i;
    }
  ...
  for(int i=in; i<cols; i++) {
	...
	work[k] = work[k]+D[j]*r[pos1]*r[pos2];
	pos2++;
      }
...
void lsq(double** A, double* weights, double* y, int rows, int cols) {
  ...
  double sserr[1], D[cols], r[r_dim], rss[cols], rhs[cols], xrow[cols+1], work[cols], tolerances[cols], ycorr[MAXVAR], cormat[max_cdim];
  ...
  in = 1;
  partial_corr(cols, ifault, in, max_cdim, D, r, sserr, work, rhs, cormat, ycorr);
